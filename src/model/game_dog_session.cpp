...
namespace model
{
    ...
    bool Dog::IsNeededTStopDog(const DogCoordinates &coords, const Limits &lim)
    {
    ...
    std::string Dog::GrabName() { return std::move(name_); }
    ...
    void Dog::TakeScores(int score)
    {
        score_ += score;
    }

    void Dog::PickUpLoot(Loot loot)
    {
    ...
    const std::vector<Loot> &Dog::GetLootBag() const
    {
        return bag_;
    }
