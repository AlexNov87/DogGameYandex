...
std::ostream &operator<<(std::ostream &os, const model::Road &road)
{
    ...
    if (road.IsHorizontal())
    {
        ...
        assert(road.GetStart().y == road.GetEnd().y);
        os << "Road Start X: " << std::min(road.GetStart().x, road.GetEnd().x)
           << "  End X: " << std::max(road.GetStart().x, road.GetEnd().x) << " Y: "
           << road.GetEnd().y << "  HORIZONTAL";
    }
    else
    {
        assert(road.GetStart().x == road.GetEnd().x);
        os << "Road Start Y: " << std::min(road.GetStart().y, road.GetEnd().y)
           << "  End Y: " << std::max(road.GetStart().y, road.GetEnd().y)
           << " X: " << road.GetEnd().x << "  VERTICAL";
    }
    return os;
}
