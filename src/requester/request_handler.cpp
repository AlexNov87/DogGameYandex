 ...
 VariantResponse RequestHandler::MakeStaticRespone(unsigned version, bool keep_alive, std::string target)
  {
    ...
    if (tmp.empty())
    {
      if (sys::error_code ec; file.open(index_path.string().c_str(), beast::file_mode::read, ec), ec)
      {
        std::cout << "Failed to open file "sv << index_path.string().c_str()<< std::endl;
    ...
    if (sys::error_code ec; file.open(absolute.string().c_str(), beast::file_mode::read, ec), ec)
    {
      auto resp = Make404(version, keep_alive);
      std::cout << "Failed to open file "sv << absolute.string().c_str() << std::endl;
      resp.prepare_payload();
      return resp;
    }
