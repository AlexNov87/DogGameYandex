...
namespace request_handler
{
  ...
  OffLim Offset_Limit(std::string line)
  {
    OffLim offlim;
    if (line == json_foo::records){return offlim;}

    std::unordered_map<std::string, int64_t> mp;
    std::vector<std::string> parsed;
    boost::algorithm::split(parsed, line, boost::is_any_of("?&="));

    if (parsed[0] != json_foo::records){
      throw std::logic_error("BAD RECORD");
    }
    
    for (size_t i = 1; i < parsed.size(); i += 2)
    {
      mp[parsed[i]] = std::stoi(parsed[i + 1]);
    }

    if(mp.count("start")){ offlim.offset = mp.at("start");}
    if(mp.count("maxItems")){ offlim.limit = mp.at("maxItems");}
    return offlim;
  }
...
namespace request_handler
{
  ...
  json::value DogObject(std::shared_ptr<model::Dog> dog)
  {
    ...
    tmp[std::string(req_static_str::score)] = dog->Score();
