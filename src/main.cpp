...
#include "serializer/serialize_overloads.h"
...
int main(int argc, const char *argv[])
{
    ...
    try
    {
        ...
        if (params)
        {
            ...
            //ПОДПИСЫВАЕМСЯ НА СИГНАЛ ДВИЖЕНИЯ
            game.SubscribeMoving ([&](const std::string &map, size_t id_pl) {
                   api::TCC::SignalMove(map, id_pl);
            });

            //ПОДПИСЫВАЕМСЯ НА СТОП - СИГНАЛ
            game.SubscribeStop([&](const std::string &map, size_t id_pl, const std::chrono::system_clock::time_point& servertime) {
                   api::TCC::SignalStop(map, id_pl, servertime);
            });
                
                // ПРИ НЕОБХОДИМОСТИ СОЗДАЕМ КЛАСС СОХРАНЕНИЯ ИГРЫ
                std::shared_ptr<serialization::ArchivePlay>
                    play_saver; // пока обьявим указалель
                                // далее заполним значением есои потребуется

            // ЕСЛИ ЕСТЬ ПУТЬ К ФАЙЛУ С СОХРАНЕНИЕМ
            if (params.value().path_to_save.has_value())
            {

                // params.value().path_to_save.value() = params.value().path_to_save.value().lexically_normal();
                // ДЕЛАЕМ СТРУКТУРУ С СОХРАНЕННЫМИ ДАННЫМИ
                auto game_old = serialization::LoadOldGame(params.value().path_to_save.value());
                // ЕСЛИ ПОЛУЧИЛОСЬ СДЕЛАТЬ СТРУКТУРУ - ТО ИЗ НЕЕ ВОССТАНАВЛИВАЕМ ИГРУ
                if (game_old.has_value())
                {
                    serialization::DearchivePlay dearchive(game);
                    game_old.value().RestoreGAME(dearchive);
                }

                // ЗАПОЛНЯЕМ play_sаver ЗНАЧЕНИЕМ
                play_saver = std::make_shared<serialization::ArchivePlay>(game, params.value().path_to_save.value());

                // ЕСЛИ НАДО ПЕРИОДИЧЕСКИ СОХРАНЯТЬ ИГРУ
                if (params.value().save_period.has_value())
                {
                    play_saver->SetSavePerod(params.value().save_period.value());
                    // ПРИВЯЗЫВАЕМ К ИГРЕ СЛУШАТЕЛЯ СИГНАЛА
                    game.SubsribeFunctionToGameSignal([ps = play_saver->shared_from_this()](std::chrono::milliseconds ms)
                                                      { ps->NeedWrite(ms); });
                }
            }
     ...
     if (!ec) {
        ...
        logger::LogStop(ec.value());
            ...
            auto lam_to_serve = [handler](auto &&req, auto &&send)
                                   { 
                                         //Handler из main  
                                         /*
                                         На вход принимает запрос req и функцию которая отправит ответ.
                                         auto &&req, auto &&send придут позже.
                                         */
           ...
           // 5. Запустить обработчик HTTP-запросов, делегируя их обработчику запросов
            http_server::ServeHttp(ioc, endpoint, lam_to_serve); 
            ...
            // 7.При необходимости сохраняем в файл
            if (play_saver)
            {
                play_saver->Archive();
            }
